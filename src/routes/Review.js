...
router.post("/", async (req, res) => {
  const { userId, title, comment, score, productId } = req.body;
  ...
  if (!(await userBuyProduct(userId, productId)))
    return res
      .status(400)
      .send({ error: "User not buy this product", productId });
  ...
  if (
    !productId ||
    !/^[0-9a-fA-F]{8}\b-([0-9a-fA-F]{4}-){3}\b[0-9a-fA-F]{12}$/.test(productId)
  ) {
  ...
  const newReview = await createReview(userId, title, score, comment);

  const review = await product.addReview(newReview).catch((e) => {
    console.log(e);
    return false;
  });
  if (review) {
    res
      .status(200)
      .send({ success: "Successfully added review.", result: newReview.id });
  } else {
    res
      .status(503)
      .send({ error: "There was a problem processing your request." });
  }
});
